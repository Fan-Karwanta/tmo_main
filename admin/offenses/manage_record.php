...
if(!isset($ticket_no)){
    $last_ticket = $conn->query("SELECT ticket_no FROM `offense_list` ORDER BY CAST(ticket_no AS UNSIGNED) DESC LIMIT 1");
    if($last_ticket->num_rows > 0){
        $last_number = intval($last_ticket->fetch_assoc()['ticket_no']);
        $ticket_no = $last_number + 1;
    } else {
        $ticket_no = 100001;
    }
}
        ...
        <div class="row">
            <div class="col-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="ticket_no" id="ticket_no" value="<?php echo isset($ticket_no) ? $ticket_no : '' ?>" readonly required>
                </div>
                <div class="form-group">
            ...
            <div class="col-6">
                <div class="form-group">
                    <lable class="control-label" for="officer_id">Officer</lable>
                    <select name="officer_id" id="officer_id" class="custom-select select2" required onchange="updateOfficerName(this.value)">
                        <option value=""></option>
                        <?php 
                        $officers = $conn->query("SELECT * FROM `officers` order by `name` asc ");
                        while($row = $officers->fetch_assoc()):
                        ?>
                        <option value="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" <?php echo (isset($officer_id) && $officer_id == $row['id']) ? 'selected' : '' ?>><?php echo ucwords($row['name']) ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="form-group">
                    <lable class="control-label" for="officer_name">Officer Name</lable>
                    <input type="text" class="form-control" name="officer_name" id="officer_name" value="<?php echo isset($officer_name) ? $officer_name : '' ?>" readonly required>
                </div>
                <div class="form-group">
...
<script>
    ...
    function updateOfficerName(officerId) {
        var selectedOption = document.querySelector('#officer_id option[value="' + officerId + '"]');
        if(selectedOption) {
            document.getElementById('officer_name').value = selectedOption.getAttribute('data-name');
        } else {
